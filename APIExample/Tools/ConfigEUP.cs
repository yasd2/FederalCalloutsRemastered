...
namespace FederalCallouts.Tools
{
    ...
    public class EUPOutfit
    {
        ...
        {
            // LogWrapper.LogDebug("Attempting to convert " + compSetting + " to component entry");
        ...
        public bool ApplyOutfitToPlayer(bool allowChangeModel, bool allowChangeModelInVehicle = true, bool restoreWeapons = true, bool randomizeFace = false)
        {
            ...
            int prevSeat = -1;
            var inventoryBefore = Helper.Player.Inventory.Weapons.ToDictionary(w => w.Hash, w => w.Ammo);
            ...
            if (Game.LocalPlayer.Model != requiredModel)
            {
                ...
                GameFiber.StartNew(delegate
                {
                    ...
                    if (restoreWeapons)
                    {
                        ...
                        {
                            Game.LogDebug("Restoring weapon " + wpn.Key.ToString());
        ...
        public bool ApplyOutfitToPed(Ped ped, bool randomizeFace)
        {
            ...
            {
                Game.LogTrivialDebug("Setting component " + (int)comp.Key + " (" + comp.Key.ToString() + ") to model " + comp.Value.DrawableVariation + ", texture " + comp.Value.TextureVariation);
                ped.SetVariation((int)comp.Key, comp.Value.DrawableVariation, comp.Value.TextureVariation);
            }

            if (randomizeFace)
            {
                Game.LogTrivialDebug("Not supported.");

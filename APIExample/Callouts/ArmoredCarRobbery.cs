        ...
        public override void Process()
        {
            ...
            if (state == CarRobberyState.EnRoute && (double)Vector3Extension.DistanceTo(Game.LocalPlayer.Character.Position, securicar.Position) <= 60.0f)
            {
                ...
                GameFiber.StartNew((() =>
                {
                    ...
                    {
                        GameFiber.Wait(1000);
                        Dispatch.PlayerSay("I have eyes on the robbers!");
                    }
                    GameFiber.Wait(2000);
                    Dispatch.Copy();
                }));
            ...
            bool c1, c2, c3, c4;
            if (attacker1.Exists()) { c1 = true; } else { c1 = false; }
            if (attacker2.Exists()) { c2 = true; } else { c2 = false; }
            if (attacker3.Exists()) { c3 = true; } else { c3 = false; }
            if (attacker4.Exists()) { c4 = true; } else { c4 = false; }
